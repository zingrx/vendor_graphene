...
func init() {
	android.RegisterModuleType("graphene_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = grapheneExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = grapheneExpandVariables(ctx, dep_file)
	...
	cmd := grapheneExpandVariables(ctx, String(g.properties.Cmd))
